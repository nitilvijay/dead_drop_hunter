	...
	if (nrhs == 3)
	{
		...
		for(int fieldIndex=0; fieldIndex<nfields; fieldIndex++)
		{
			...
			if (strcmp(fieldName, "gamma") == 0)
				if (mxIsClass(fieldContent, "single")) c_gamma = (float)mxGetScalar(fieldContent);
				else mexErrMsgTxt ("'config.gamma' must be of type 'single'");
			if (strcmp(fieldName, "sigma") == 0)
				if (mxIsClass(fieldContent, "single")) c_sigma = (float)mxGetScalar(fieldContent);
				else mexErrMsgTxt ("'config.sigma' must be of type 'single'");
